  ...
  <section class="page-shell">
    ...
    <header class="d-flex align-center justify-space-between mb-8">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis">管理全局数据库连接资源，配置连接信息与逻辑外键。</p>
      </div>
    ...
    <v-card variant="flat" border class="rounded-lg">
      ...
      >
        ...
        <template v-slot:item.testStatus="{ item }">
          <v-chip
            :color="item.testStatus === 'success' ? 'success' : (item.testStatus === 'fail' ? 'error' : 'default')"
            size="small"
            variant="flat"
            class="px-3"
          >
            <span class="d-flex align-center">
              <span v-if="item.testStatus === 'success'" class="breathing-dot-green"></span>
          ...
          <tr>
            <td :colspan="columns.length" class="bg-grey-lighten-5 pa-0">
              <div class="expand-row-content pa-6">
                ...
                <div class="d-flex align-center justify-space-between mb-4">
                  <div class="d-flex align-center">
                    ...
                    <span class="text-caption text-medium-emphasis ml-4">
                      已选择 {{ (item.id ? selectedTables[item.id]?.length : 0) ?? 0 }} 个表
                    </span>
                  </div>
                  <div class="ga-2 d-flex">
                    <v-btn variant="text" size="small" @click="selectAllTables(item)">全选</v-btn>
                    <v-btn variant="text" size="small" @click="clearAllTables(item)">清空</v-btn>
                    <v-btn
                      color="primary"
                      size="small"
                      elevation="0"
                ...
                <div v-if="item.id && loadingTablesId === item.id" class="d-flex justify-center py-8">
                  <v-progress-circular indeterminate color="primary" size="32"></v-progress-circular>
                </div>
                <div v-else-if="item.id && tableFetchError[item.id]" class="empty-state error-state">
                  ...
                  <v-btn color="primary" variant="outlined" size="small" @click="retryFetchTables(item)">
                    重试
                  </v-btn>
                </div>
                ...
                <div v-if="!loadingTablesId && !(item.id && tableFetchError[item.id]) && (item.id ? (tableLists[item.id]?.length ?? 0) : 0) === 0" class="empty-state">
    ...
    <v-dialog v-model="dialog.visible" max-width="800" persistent>
      <v-card rounded="xl" class="pa-2">
        <v-card-title class="d-flex align-center justify-space-between px-4 pt-4">
          <span class="text-h6 font-weight-bold">{{ dialog.mode === 'create' ? '添加数据源' : '编辑数据源' }}</span>
        ...
        <v-card-text class="pt-4">
          <v-form ref="formRef" v-model="formValid">
            <v-row dense>
              ...
              <v-col cols="12" md="6">
                ...
                <v-select 
                  v-model="form.type" 
                  :items="['mysql', 'postgresql', 'sqlserver', 'dameng', 'oracle']" 
                  variant="outlined" 
                  density="compact" 
                  :rules="[rules.required]"
                />
              ...
              <v-col cols="12" md="6" v-if="['postgresql', 'oracle'].includes(form.type || '')">
          ...
          <div class="d-flex justify-end mt-4 ga-3">
            <v-btn variant="text" class="text-none" @click="closeDialog">取消</v-btn>
            <v-btn 
              color="primary" 
      ...
      <v-card rounded="xl" class="pa-4">
        <v-card-title class="d-flex align-center justify-space-between border-b pb-4">
          <div class="d-flex align-center">
            ...
            <span class="font-weight-bold">逻辑外键配置 - {{ fkDialog.datasourceName }}</span>
          </div>
          <v-btn icon="mdi-close" variant="text" size="small" @click="fkDialog.visible = false"></v-btn>
        </v-card-title>
...
const isRelationFormValid = computed(() => {
  return fkForm.sourceTableName && fkForm.sourceColumnName && 
         fkForm.targetTableName && fkForm.targetColumnName && 
         fkForm.relationType;
});
...
const fetchColumns = async (tableName: string, type: 'source' | 'target') => {
  ...
  try {
    const columns = await datasourceService.getTableColumns(fkDialog.datasourceId, tableName);
    if (type === 'source') {
...
const handleAddRelation = async () => {
  ...
  try {
    const res = await datasourceService.addLogicalRelation(fkDialog.datasourceId, {
      ...fkForm,
      description: ''
    });
...
const handleDeleteRelation = (relationId: number) => {
  ...
  showConfirm({
    ...
    onConfirm: async () => {
      ...
      try {
        const res = await datasourceService.deleteLogicalRelation(fkDialog.datasourceId, relationId);
        if (res.success) {
